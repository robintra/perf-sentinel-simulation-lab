...
- 6 Redis children (`db.system=redis`, `GET user:1..6`) — must be dropped.
- 1 Elasticsearch child carrying **both** `db.statement` and `url.full` — must
  be dropped on `db.system`, **not** reclassified as an HTTP finding (the edge
  the task asks to check first on the OTLP path).
...
- **OTLP daemon**: a throwaway loopback `perf-sentinel watch` daemon ingests
  `mixed.pb`. Findings carry only the PostgreSQL N+1;
  `perf_sentinel_otlp_spans_filtered_total{reason="non_sql_datastore"}` rises
  by 7 (6 redis + 1 es) while `{reason="not_io"}` stays 0 — proving the
  elasticsearch span with `url.full` was dropped on `db.system`, not
  re-bucketed as HTTP.
...
(`cargo build --release -p perf-sentinel`). No cluster, no port-forward — the
OTLP leg launches its own loopback daemon (ports 14392/14393, override with
`DAEMON_HTTP_PORT`/`DAEMON_GRPC_PORT`).
