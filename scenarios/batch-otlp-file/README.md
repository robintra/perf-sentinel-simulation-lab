...
- `fixtures/java-stdout-wrapper.json` is a verbatim copy of opentelemetry-java's
  own expected output for the `experimental-otlp/stdout` exporter
  (`exporters/logging-otlp/src/test/resources/expected-spans-wrapper.json`,
  Apache-2.0). A8 reconstitutes a Failsafe `-output.txt` around it rather than
  inventing a fixture, so the assertion tracks what the exporter actually emits.
  The fixture carries no I/O span, hence `traces_analyzed = 0` — A8 is an ingest
  assertion, and A9 is the one that checks the findings are untouched.
