#!/usr/bin/env bash
# B2-4 multi-format input live: traces flow through OTel Collector
...
SCENARIO="b2-4-multiformat-input"
...
step "Write report"
{
  echo "# B2-4 multi-format input (live multi-backend + boundary fixtures)"
  echo
...
if [ "${verdict_live}" = "PASS" ] && [ "${verdict_boundary}" = "PASS" ]; then
  ok "B2-4 PASS, see ${REPORT}"
  exit 0
else
  die "B2-4 partial (live=${verdict_live} boundary=${verdict_boundary}), see ${REPORT}"
fi
