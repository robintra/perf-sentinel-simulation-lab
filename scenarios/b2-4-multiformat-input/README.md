# B2-4 multi-format input (Jaeger + Zipkin live + boundary fixtures)

## Use case

The same OTel Collector pipeline emits traces in parallel to three
backends: Tempo (existing), Jaeger (added by this scenario), and Zipkin
(added by this scenario). perf-sentinel `analyze` is run on the raw
trace exports from Jaeger and Zipkin and the findings should be
...
1. **Live multi-backend**: applies `manifests.yaml` (Jaeger + Zipkin
   ...
   `/api/fault/redundant-sql?items=10`), polls the backends until rich
   traces (>5 spans, not single-span health probes) appear, fetches
   them via the Jaeger and Zipkin query APIs, runs `perf-sentinel analyze`
   on each export, and asserts at least one finding category common
...
`/tmp/scenario-b2-4-multiformat-input-report.md` plus
`/tmp/b2-4-multiformat-input/{jaeger-traces,zipkin-traces,jaeger-findings,zipkin-findings}.json`.
