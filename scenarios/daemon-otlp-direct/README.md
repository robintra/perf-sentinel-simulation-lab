# daemon receives OTLP HTTP directly (no Collector)
...
(name retained from the original sprint slug to avoid forcing a
`kubectl delete namespace` on existing deployments) with:
...
`/tmp/scenario-daemon-otlp-direct-report.md` plus
`/tmp/daemon-otlp-direct/direct-report.json` (raw daemon report).
