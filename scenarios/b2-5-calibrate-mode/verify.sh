#!/usr/bin/env bash
# B2-5 calibrate mode: energy coefficients calibration.
#
...
step "Write report"
{
  echo "# B2-5 calibrate mode (energy coefficients)"
  echo
  ...
  echo "of B2-5 was incorrect on this point."
} > "${REPORT}"

if [ "${verdict}" = "PASS" ]; then
  ok "B2-5 PASS, see ${REPORT}"
  exit 0
else
  die "B2-5 ${verdict}, see ${REPORT}"
fi
