#!/usr/bin/env bash
# B2-1 hybrid daemon -> batch enriched ingest.
#
...
#     for SARIF is the use case of B2-2 (batch over a Tempo backend).
...
#     dashboard path B2-1 validates.

set -euo pipefail

SCENARIO="b2-1-hybrid-daemon-batch"
...
step "Write report"
{
  echo "# B2-1 hybrid daemon -> batch enriched ingest"
  echo
  ...
  echo "SARIF-from-batch path is covered by scenario B2-2 (batch over Tempo)."
  echo
  echo "## Output"
  echo
...
if [ "${verdict}" = "PASS" ]; then
  ok "B2-1 PASS, see ${REPORT}"
  exit 0
else
  die "B2-1 FAIL, see ${REPORT}"
fi
