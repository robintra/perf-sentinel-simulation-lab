...
const BASE = __ENV.SERVICE_URL || 'http://ktor-svc.shop.svc.cluster.local:8097';
...
export default function() {
    const antiPattern = __ENV.ANTI_PATTERN || 'n_plus_one_sql';
    const dispatch = DISPATCH[antiPattern];
    if (!dispatch) throw new Error(`unknown ANTI_PATTERN: ${antiPattern}`);
    dispatch();
}
