// symfony-svc multistack runner. Same dispatcher pattern as the other multistack
// scenarios. Driven by scripts/run-multistack-scenario.sh via __ENV.ANTI_PATTERN.
//
// n-plus-one-sql uses items=20 (>= 3 * n_plus_one_threshold) so the daemon's strict
// sanitizer-aware classifier reclassifies the sanitized Doctrine group to
// n_plus_one_sql. SQL findings carry suggested_fix.framework = php_doctrine (the
// DBAL span rides io.opentelemetry.contrib.php.doctrine); non-SQL findings fall
// through to php_generic. See docs/MULTISTACK.md.
...
const DISPATCH = {
    n_plus_one_sql:    () => { postChecked('/api/fault/n-plus-one-sql?items=20'); sleep(0.5); },
