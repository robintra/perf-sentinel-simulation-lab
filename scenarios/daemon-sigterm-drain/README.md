...
|-----------------------|---------------------------|------------------------------------------------------------------|
| `SIGTERM_DRAIN_IMAGE` | `perf-sentinel:0.8.5-lab` | daemon image under test; set a 0.8.4 image for the counter-check |
...
- A `0.8.5` daemon image imported into k3d (see `scripts/k3d-image.sh`); until the
  official `0.8.5` digest is published, build it from the upstream checkout and
  `k3d image import perf-sentinel:0.8.5-lab -c perf-sentinel-lab`.
