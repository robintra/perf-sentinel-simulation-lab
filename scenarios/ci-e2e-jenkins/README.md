...
> **Quarantined: this scenario currently FAILs on J0.** It has a
> `make verify-ci-e2e-jenkins` target and is covered by `make validate`, but it
> is kept out of `make verify-all-scenarios` and out of the scenario count until
> it goes green. See *What it found*.
...
- A real controller built from `jenkins/jenkins:2.568.1-lts-jdk25` — the current
  LTS line, arm64-native. The perf-sentinel binary is taken from the **released**
  image `ghcr.io/robintra/perf-sentinel:0.9.24`, which is both the artifact a user
  installs and the only way to get a Linux binary from a macOS host.
