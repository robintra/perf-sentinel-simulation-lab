#!/usr/bin/env bash
# Build, import, and helm-install the symfony-svc multistack member.
# Symfony 7 + Doctrine ORM + native OpenTelemetry PHP (PHP 8.3).
set -euo pipefail
...
PASSWORD_FILE="${REPO_ROOT}/.postgres-symfony-svc-password"
if [ ! -f "${PASSWORD_FILE}" ]; then
  printf 'lab_symfony' > "${PASSWORD_FILE}"
