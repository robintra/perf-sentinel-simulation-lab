#!/usr/bin/env bash
# Build, import, and helm-install the mutiny-svc multistack member
# (services/mutiny-svc). Sibling of scripts/seed-quarkus-svc.sh.
#
# Usage: ./scripts/seed-mutiny-svc.sh
set -euo pipefail
...
PASSWORD_FILE="${REPO_ROOT}/.postgres-mutiny-svc-password"
if [ ! -f "${PASSWORD_FILE}" ]; then
  printf 'lab_mutiny' > "${PASSWORD_FILE}"
