#!/usr/bin/env bash
# Build, import, and helm-install the dotnet-svc multistack member.
# .NET 10 LTS + EF Core 10 + Npgsql.
#
# Usage: ./scripts/seed-dotnet-svc.sh
set -euo pipefail
...
PASSWORD_FILE="${REPO_ROOT}/.postgres-dotnet-svc-password"
if [ ! -f "${PASSWORD_FILE}" ]; then
  printf 'lab_dotnet' > "${PASSWORD_FILE}"
