#!/usr/bin/env bash
# Build, import, and helm-install the helidon-se-svc multistack member.
# Sibling of scripts/seed-helidon-mp-svc.sh.
#
# Usage: ./scripts/seed-helidon-se-svc.sh
set -euo pipefail
...
PASSWORD_FILE="${REPO_ROOT}/.postgres-helidon-se-svc-password"
if [ ! -f "${PASSWORD_FILE}" ]; then
  printf 'lab_helidon_se' > "${PASSWORD_FILE}"
