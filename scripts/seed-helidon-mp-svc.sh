#!/usr/bin/env bash
# Build, import, and helm-install the helidon-mp-svc multistack member
# (services/helidon-mp-svc). Sibling of scripts/seed-quarkus-svc.sh and
# scripts/seed-mutiny-svc.sh.
#
# Usage: ./scripts/seed-helidon-mp-svc.sh
set -euo pipefail
...
PASSWORD_FILE="${REPO_ROOT}/.postgres-helidon-mp-svc-password"
if [ ! -f "${PASSWORD_FILE}" ]; then
  printf 'lab_helidon_mp' > "${PASSWORD_FILE}"
