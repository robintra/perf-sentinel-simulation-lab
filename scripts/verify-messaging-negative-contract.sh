...
run_focused_test() {
    ...
    if ! grep -Fxq "${MARKER}" <<< "${output}"; then
        echo "focused test exited 0 without exact marker: ${MARKER}" >&2
        return 1
    fi
}
...
case "${STACK}" in
    quarkus)
        ...
        run_focused_test mvn -B -ntp -f services/quarkus-svc/pom.xml -Dtest=MessagingInvalidContractTest test
        ;;
    mutiny)
        ...
        run_focused_test mvn -B -ntp -f services/mutiny-svc/pom.xml -Dtest=MessagingInvalidContractTest test
        ;;
    helidon-se)
        ...
        run_focused_test mvn -B -ntp -f services/helidon-se-svc/pom.xml -Dtest=MessagingInvalidContractTest test
        ;;
    helidon-mp)
        ...
        run_focused_test mvn -B -ntp -f services/helidon-mp-svc/pom.xml -Dtest=MessagingInvalidContractTest test
        ;;
    ktor)
        require_focused_test services/ktor-svc/src/test/kotlin/com/perfsim/ktor/ApplicationTest.kt messagingInvalidContract
        run_focused_test mvn -B -ntp -f services/ktor-svc/pom.xml -Dtest=ApplicationTest#messagingInvalidContract -Dsurefire.failIfNoSpecifiedTests=true test
        ;;
    django)
        ...
        run_focused_test docker run --rm --entrypoint python django-svc:plan-check -m unittest djangosvc.tests.test_messaging_invalid_contract
        ;;
    fastapi)
        ...
        run_focused_test docker run --rm --entrypoint python fastapi-svc:plan-check -m unittest app.test_messaging_invalid_contract
        ;;
    laravel)
        ...
        run_focused_test docker run --rm --entrypoint php laravel-svc:plan-check /app/tests/messaging-invalid-contract.php
        ;;
    symfony)
        ...
        run_focused_test docker run --rm --entrypoint php symfony-svc:plan-check /app/tests/messaging-invalid-contract.php
        ;;
    diesel)
        require_focused_test services/diesel-svc/src/main.rs messaging_invalid_contract
        run_focused_test cargo test --manifest-path services/diesel-svc/Cargo.toml messaging_invalid_contract
        ;;
    seaorm)
        require_focused_test services/seaorm-svc/src/main.rs messaging_invalid_contract
        run_focused_test cargo test --manifest-path services/seaorm-svc/Cargo.toml messaging_invalid_contract
        ;;
    rails)
        ...
        run_focused_test docker run --rm -e RAILS_ENV=test -e OTEL_SDK_DISABLED=true rails-svc:plan-check bin/rails test test/controllers/messaging_invalid_contract_test.rb
        ;;
    nest)
        require_focused_test services/nest-svc/src/fault/messaging-invalid-contract.spec.ts
        run_focused_test npm run test:messaging-negative --prefix services/nest-svc
        ;;
    dotnet)
        ...
        run_focused_test dotnet test services/dotnet-svc.tests/DotnetSvc.Tests.csproj --no-restore --filter FullyQualifiedName~MessagingInvalidContract
        ;;
    go)
