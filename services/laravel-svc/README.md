...
service (`SELF_BASE_URL`), and a single-worker server deadlocks on those reentrant self-calls.
OTel auto-instrumentation (`opentelemetry-auto-laravel` + `opentelemetry-auto-pdo`) is enabled
via `OTEL_PHP_AUTOLOAD_ENABLED=true`; OTel config is passed as **real env vars** (the SDK
autoloader reads `getenv()` before Laravel loads `.env`).
...
`/health/live` `/health/ready`. SQL faults go through Eloquent; HTTP faults emit their own CLIENT
spans (carrying `url.full`) via `curl` / `curl_multi`.
