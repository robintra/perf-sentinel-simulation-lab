<?php
// Idempotent schema + seed bootstrap, mirroring laravel-svc / rails-svc.
// Creates the 3 tables in the `symfony` schema and seeds 100 orders / 500 order
// items / 200 payments only when empty. Guarded by a per-service PostgreSQL
// advisory lock. Raw PDO, independent of the Symfony/Doctrine bootstrap.
declare(strict_types=1);

$host   = getenv('DB_HOST') ?: 'localhost';
$port   = getenv('DB_PORT') ?: '5432';
$dbname = getenv('DB_NAME') ?: 'lab';
$user   = getenv('DB_USER') ?: 'symfony_user';
$pass   = getenv('DB_PASSWORD') ?: 'lab_symfony';
$schema = getenv('DB_SCHEMA') ?: 'symfony';
$lock   = 808096; // unique per service (rails 808094, laravel 808095)
...
for ($attempt = 1; $attempt <= 60; $attempt++) {
    try {
        ...
        if ($attempt === 60) {
            fwrite(STDERR, "symfony-svc: postgres not reachable: {$e->getMessage()}\n");
            exit(1);
        }
        sleep(2);
    }
...
fwrite(STDOUT, "symfony-svc: schema ready in {$schema}\n");
