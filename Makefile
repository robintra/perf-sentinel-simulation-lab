...
validate: ## Validate manifests, helm values, dashboards, scripts (no cluster)
	...
	@python3 -c 'import re; srcs={"daemon":open("manifests/perf-sentinel-daemon.yaml").read(),"hub":open("manifests/perf-sentinel-hub.yaml").read()}; bad=[k+": image is not a GHCR digest pin" for k,s in srcs.items() if not re.search(r"image: ghcr\.io/robintra/perf-sentinel(-hub)?@sha256:",s)]+[k+": imagePullPolicy Never" for k,s in srcs.items() if "imagePullPolicy: Never" in s]+["daemon: [daemon.hub_export] enabled = true, the Secret only exists after seed-hub-local.sh" for _ in [0] if re.search(r"\[daemon\.hub_export\]\s*\n\s*enabled = true",srcs["daemon"])]; assert not bad, "local seed-*-local.sh working-tree edits leaked into the committed manifests: "+"; ".join(bad)'
